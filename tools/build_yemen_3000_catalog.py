...
def off_products(target: int) -> list[dict]:
    ...
    while len(out) < target and page <= 45:
        ...
        for p in data.get('products', []):
            ...
            name = (p.get('product_name_ar') or p.get('product_name') or '').strip()
            ...
            out.append({
                ...
                'category': (cats[0].replace('-', ' ') if cats else 'مواد غذائية'),
                'packSize': str(p.get('quantity') or '').strip(),
                'source': 'Open Food Facts',
                'sourceUrl': f'https://world.openfoodfacts.org/product/{code}',
                'sourceType': 'open_food_facts_fallback',
                'imageUrls': [image],
                'barcode': code,
                'priceYER': None,
...
def choose_catalog(target: int) -> list[dict]:
    ...
    for p in seed:
        ...
        q.setdefault('priceYER', None)
        q.setdefault('openingQuantity', None)
        q.setdefault('imageUrls', [])
        catalog.append(q)
        ids.add(q['id'])
...
def augment(im: Image.Image, n: int, rng: random.Random) -> list[Image.Image]:
    ...
    for _ in range(n):
        ...
        if rng.random() < 0.75:
            ...
            bg = Image.new('RGB', (320, 320), tuple(rng.randint(0, 255) for _ in range(3)))
            bg.paste(x, (rng.randint(0, 320 - nw), rng.randint(0, 320 - nh)))
            x = bg
        else:
...
def main():
    ...
    ap.add_argument('--target-products', type=int, default=3000)
    ...
    stats = {
        ...
        'trainedProducts': len(trained),
        'embeddingDimension': 512,
        'storedVectors': int(matrix.shape[0]),
        'referenceVariantsPerTrainedProduct': args.augments + 1,
        'hydratedImages': hydrated,
        'encoder': 'MobileCLIP2-S0 Vision ONNX',
        'catalogImageSources': ['Yemen seed catalog', 'Open Food Facts'],
        'rights': 'Open Food Facts data/images carry source-specific licenses; provenance is retained and commercial redistribution must comply with the applicable license/attribution terms.',
        'embeddingSeconds': round(time.perf_counter() - t0, 2),
