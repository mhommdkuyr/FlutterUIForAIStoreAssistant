...
UA='AIStoreAssistant-YemenCatalog/2.4'; IMG='https://images.openfoodfacts.org/images/products'
...
def fallback_products(target):
 columns=['code','product_name','generic_name','brands','categories_tags','quantity','images']
 ds=load_dataset('openfoodfacts/product-database',split='food',streaming=True)
 ds=ds.select_columns(columns)
 out=[]; seen=set(); scanned=0
 for p in ds:
  scanned+=1
  ...
  out.append({'id':'off-'+re.sub(r'[^0-9A-Za-z_-]','',code),'nameAr':name,'brand':first_value(p.get('brands')),'category':cat.replace('en:','').replace('-',' '),'packSize':first_value(p.get('quantity')),'source':'Open Food Facts Parquet','sourceUrl':f'https://world.openfoodfacts.org/product/{code}','sourceType':'open_food_facts_parquet','imageUrls':[image],'barcode':code,'priceYER':None,'openingQuantity':None})
  seen.add(code)
  if len(out)>=target:break
  if scanned>=500000:break
 if len(out)<target:raise RuntimeError(f'Only {len(out)} OFF streaming products discovered after scanning {scanned} rows')
 return out
...
def main():
 ...
 stats={'targetProducts':a.target_products,'catalogProducts':len(catalog),'trainedProducts':len(trained),'embeddingDimension':512,'storedVectors':int(matrix.shape[0]),'referenceVariantsPerTrainedProduct':a.augments+1,'hydratedImages':hydrated,'encoder':'MobileCLIP2-S0 Vision ONNX','catalogImageSources':['Yemen seed catalog','Open Food Facts Parquet via Hugging Face datasets streaming'],'rights':'Open Food Facts data is ODbL and product images CC-BY-SA; preserve attribution and comply with licenses before commercial redistribution.','embeddingSeconds':round(time.perf_counter()-t0,2),'builtAt':time.strftime('%Y-%m-%dT%H:%M:%SZ',time.gmtime())}
 (OUT/'build_stats.json').write_text(json.dumps(stats,ensure_ascii=False,indent=2),encoding='utf-8'); (OUT/'source_licenses.md').write_text('# Provenance and licenses\n\nFallback products are from Open Food Facts Parquet via Hugging Face datasets streaming. Data is ODbL and product images CC-BY-SA; preserve attribution and comply with applicable licenses before commercial redistribution.\n',encoding='utf-8'); print(json.dumps(stats,ensure_ascii=False,indent=2))
