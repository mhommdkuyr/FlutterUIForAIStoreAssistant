...
void main() {
  group('MobileClip2ModelContract actual metadata schema', () {
    ...
    test('modified mean fails closed', () {
      ...
      invalid['normalization'] = {'mean': [0.48145566, 0.4578275, 0.40821073], 'std': [0.26862954, 0.26130258, 0.27577711]};
      expect(() => MobileClip2ModelContract.fromJson(invalid), throwsStateError);
    });
    test('modified std fails closed', () {
      final invalid = _actualMetadata();
      invalid['normalization'] = {'mean': [0.48145466, 0.4578275, 0.40821073], 'std': [0.26862954, 0.26130358, 0.27577711]};
      expect(() => MobileClip2ModelContract.fromJson(invalid), throwsStateError);
    });
    ...
    test('invalid mean/std fails closed', () {
      ...
      invalidMean['normalization'] = {'mean': [0.1, double.nan, 0.3], 'std': [0.2, 0.2, 0.2]};
      expect(() => MobileClip2ModelContract.fromJson(invalidMean), throwsStateError);
      final invalidStd = _actualMetadata();
      invalidStd['normalization'] = {'mean': [0.1, 0.2, 0.3], 'std': [0.2, 0.0, 0.2]};
  ...
  group('MobileCLIP2 graph shape helpers', () {
    test('[1,3,224,224] is NCHW', () => expect(MobileClip2ModelContract.detectGraphLayout([1, 3, 224, 224], 224), 'NCHW'));
    test('[1,224,224,3] is NHWC', () => expect(MobileClip2ModelContract.detectGraphLayout([1, 224, 224, 3], 224), 'NHWC'));
    test('dynamic batch [-1,3,224,224] is valid NCHW', () => expect(MobileClip2ModelContract.detectGraphLayout([-1, 3, 224, 224], 224), 'NCHW'));
    test('invalid input graph shape fails closed', () => expect(() => MobileClip2ModelContract.detectGraphLayout([1, 3, 256, 256], 224), throwsStateError));
    test('static output shape [1,512] is valid', () => expect(MobileClip2ModelContract.isSingleEmbeddingShape([1, 512], 512), isTrue));
    test('dynamic output shape [-1,512] is valid', () => expect(MobileClip2ModelContract.isSingleEmbeddingShape([-1, 512], 512), isTrue));
