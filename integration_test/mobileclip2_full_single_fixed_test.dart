  ...
  testWidgets('MobileCLIP2 real Android camera + recognition validation', (tester) async {
    ...
    try {
      ...
      for (var id = 0; id < productCount; id++) {
        ...
        products.add(ProductModel(id: 'product-$id', name: 'Validation Product $id', category: 'MobileCLIP2 validation', purchasePrice: 1, sellingPrice: 2, quantity: 10, imageUrl: path, createdAt: DateTime(2026), updatedAt: DateTime(2026)));
      }
      ...
      for (var id = 0; id < productCount; id++) {
        for (var variant = 0; variant < 3; variant++) {
          ...
          await File(queryPath).writeAsBytes(img.encodeJpg(_makeQueryVariant(source, variant), quality: 45 + variant * 20));
          ...
          final result = index.evaluate(embedding, topK: 3, minConfidence: 0.45, minMargin: 0.08);
          totalQueries++;
      ...
      final description = cameras.firstWhere((c) => c.lensDirection == CameraLensDirection.back, orElse: () => cameras.first);
      camera = CameraController(description, ResolutionPreset.medium, enableAudio: false, imageFormatGroup: ImageFormatGroup.yuv420);
      ...
      final liveEmbedding = await provider.embedFrameWithRotation(liveFrame!, rotationDegrees: _rotationForCamera(camera, description)).timeout(const Duration(seconds: 15));
      frameWatch.stop();
      ...
      final pipeline = RecognitionPipeline(embeddingService: provider, config: const RecognitionConfig(minConfidence: 0.45, minMargin: 0.08));
      await pipeline.initialize();
      await pipeline.buildIndex(products);
      final report = await pipeline.processFrame(liveFrame!, rotationDegrees: _rotationForCamera(camera, description));
...
img.Image _makeProductImage(int id) {
  ...
  for (var y = 0; y < 320; y++) for (var x = 0; x < 320; x++) { final noise = ((x * 17 + y * 13 + id * 29) % 11) - 5; image.setPixelRgb(x, y, (bg[0] + noise).clamp(0, 255), (bg[1] + noise).clamp(0, 255), (bg[2] + noise).clamp(0, 255)); }
  final left = 55 + (id % 4) * 8, top = 45 + (id % 3) * 9, right = 265 - (id % 5) * 7, bottom = 275 - (id % 4) * 8;
  for (var y = top; y < bottom; y++) for (var x = left; x < right; x++) { final stripe = ((x + y + id * 19) ~/ (7 + id % 5)) % 2; image.setPixelRgb(x, y, stripe == 0 ? fg[0] : fg[0] ~/ 2, stripe == 0 ? fg[1] : fg[1] ~/ 2, stripe == 0 ? fg[2] : fg[2] ~/ 2); }
  for (var row = 0; row < 7; row++) for (var col = 0; col < 7; col++) { if (((id + 3) * 17 + row * 13 + col * 7) % 5 == 0) continue; final x0 = left + 16 + col * 25, y0 = top + 18 + row * 25; for (var y = y0; y < y0 + 10; y++) for (var x = x0; x < x0 + 10; x++) image.setPixelRgb(x, y, 248, 248, 248); }
  return image;
}

img.Image _makeQueryVariant(img.Image source, int variant) {
  var result = source.clone();
  if (variant == 0) { result = img.copyCrop(result, x: 22, y: 16, width: 276, height: 286); return img.copyResize(result, width: 320, height: 320, interpolation: img.Interpolation.linear); }
  if (variant == 1) { for (var y = 0; y < result.height; y++) for (var x = 0; x < result.width; x++) { final p = result.getPixel(x, y); final delta = ((x * 5 + y * 3) % 17) - 8; result.setPixelRgb(x, y, (p.r + delta + 10).clamp(0, 255).toInt(), (p.g + delta + 10).clamp(0, 255).toInt(), (p.b + delta + 10).clamp(0, 255).toInt()); } return result; }
