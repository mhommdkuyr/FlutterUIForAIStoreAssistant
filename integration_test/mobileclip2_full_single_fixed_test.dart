...
void main() {
  ...
  testWidgets('FULL Android visual engine end-to-end validation', (tester) async {
    ...
    try {
      ...
      final first = await provider.embedFile(probePath).timeout(const Duration(seconds: 20));
      ...
      final second = await provider.embedFile(probePath).timeout(const Duration(seconds: 20));
      ...
      for (var i = 0; i < productCount; i++) {
        final embedding = await provider.embedFile(referencePaths[i]).timeout(const Duration(seconds: 20));
      ...
      final robustnessReference = img.decodeImage(await File(referencePaths.first).readAsBytes())!;
      final referenceEmbedding = await provider.embedFile(referencePaths.first).timeout(const Duration(seconds: 20));
      ...
      for (var variant = 0; variant < 4; variant++) {
        ...
        await File(path).writeAsBytes(img.encodeJpg(transformed, quality: 40 + variant * 8));
        final embedding = await provider.embedFile(path).timeout(const Duration(seconds: 20));
        expect(embedding, isNotNull);
        final similarity = provider.similarity(referenceEmbedding!, embedding!);
        expect(similarity, greaterThan(0.45));
      }

      final cameras = await availableCameras().timeout(const Duration(seconds: 15));
      ...
      final cameraEmbedding = await provider.embedFile(capture.path).timeout(const Duration(seconds: 20));
